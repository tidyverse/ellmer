...
* `Chat$chat()` (and `$chat_async()`, `$chat_structured()`, `$chat_structured_async()`, `$stream()`, and `$stream_async()`) now restore the previous conversation state if a request fails partway through the tool loop, so a failed call no longer leaves the chat with a dangling tool request that breaks every later call. Note that rollback discards turns whose tool calls already executed — retrying the chat may re-run non-idempotent tools — and the discarded turns' token usage is not reflected in `$get_tokens()` or `$get_cost()`. Set `options(ellmer_preserve_turns_on_error = TRUE)` to keep the partial turns instead, e.g. for debugging.
...
* `chat_anthropic()` now reuses the code-execution container across turns within a conversation, so programmatic tool calling (`tool(allowed_callers = )`) and `claude_tool_code_execution()` can build on sandbox state from earlier turns. Expired containers are not reused, so the API provisions a fresh one rather than silently failing code execution. It also no longer sends `cache_control` on programmatically-called tool results, which the API rejects.
* `chat_anthropic()` now labels code-execution tool cards correctly when programmatic tool calling splits a code-execution request and its result across turns; previously the result that resumed after an intervening programmatic tool call rendered with an empty title and call (e.g. in shinychat).
* `chat_anthropic()` now summarises code-execution file operations: creating a file shows "File created." and an `str_replace` edit shows its diff, where previously these results rendered with an empty body.
