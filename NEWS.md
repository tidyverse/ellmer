...
* `chat_anthropic()` and `chat_aws_bedrock()` now default `base_url` to the `ANTHROPIC_BASE_URL`, `AWS_ENDPOINT_URL_BEDROCK_RUNTIME`, and `AWS_ENDPOINT_URL_BEDROCK_MANTLE` environment variables when set, matching the official SDKs (@karawoo, #1103).
