...
* `Chat` gains a `store` field — a per-conversation environment that tools can read and mutate by reference, persists across `$chat()` calls, and is copied (top-level bindings only) by `clone(deep = TRUE)` (#871).
