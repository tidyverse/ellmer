...
* New exported S7 classes `Source`, `WebSource`, and `ContentCitation` provide a provider-independent citation model. The built-in web activity classes `ContentToolRequestSearch`, `ContentToolResponseSearch`, `ContentToolRequestFetch`, and `ContentToolResponseFetch` are now also exported, with structured search sources and normalized fetch status. Citations and web activity are preserved in turns, streaming, and record/replay without sending provider annotations to incompatible model APIs. Console echo preserves progressive text output, adding citation markers as citation events arrive and a deduplicated source list when the turn completes.
...
* `chat_anthropic()` now resolves document and web-fetch citations to source URLs across the full conversation history, including prior turns (#1068).
