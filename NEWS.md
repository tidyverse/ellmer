...
* `Chat` gains a `store` field — a per-conversation environment that tools can read and mutate by reference, persists across `$chat()` calls, and is independently copied by `clone()` (#871).
