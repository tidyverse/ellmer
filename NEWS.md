...
* The OpenTelemetry `invoke_agent` span is now of kind `internal` rather than `client`, matching the GenAI semantic conventions for agents that run in-process; the model requests it wraps remain `client` spans (#1146).
* The OpenTelemetry `chat` span now records `server.address` and `server.port`, `gen_ai.output.type` for structured output requests, `gen_ai.request.reasoning.level` from `params(reasoning_effort)`, `gen_ai.usage.reasoning.output_tokens` for providers that report it (currently OpenAI), and, when `OTEL_INSTRUMENTATION_GENAI_CAPTURE_MESSAGE_CONTENT` is set, `gen_ai.tool.definitions` (@schloerke).
