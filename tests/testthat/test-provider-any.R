...
test_that("can set model or use default", {
  ...
  expect_equal(chat1$get_model()@name, chat_openai()$get_model()@name)

  chat2 <- chat("openai/gpt-4.1-mini")
  expect_equal(chat2$get_provider()@name, "OpenAI")
  expect_equal(chat2$get_model()@name, "gpt-4.1-mini")
  ...
  expect_equal(chat3$get_model()@name, "provider/model")
})
...
test_that("works for chat functions that don't include `params`", {
  ...
  expect_equal(chat$get_model()@name, "qwen3:4b")
})
