...
[Chat](https://ellmer.tidyverse.org/reference/Chat.md)'s `$stream()` or
`$stream_async()` via the `controller` argument, then call `$cancel()`
from anywhere (e.g. a Shiny observer) to stop the stream after the next
chunk arrives.
...
- `$cancel(reason = "cancelled")`: Cancel the stream. The `reason`
  ...
  [AssistantPartialTurn](https://ellmer.tidyverse.org/reference/Turn.md)'s
  `reason` property.
