...
Chat objects are [R6 objects](https://r6.r-lib.org), which means that they are **mutable**. Most R objects are immutable: when you modify them they create a copy:
...
It would be annoying if chat objects had this property, because every time you chatted with the model, you'd need to save the result in order to preserve the context of the conversation. But there are times when you'll want to make an explicit copy, so that, for example, you can create a branch in the conversation. 
...
(This is the technique that `$chat_parallel()` uses internally.)
