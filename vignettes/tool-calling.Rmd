...
This is a fair amount of code to write, even for such a simple function. Fortunately, you don't have to write this by hand! I generated the above `tool()` call by calling `create_tool_def(get_current_time)`, which printed that code at the console. `create_tool_def()` works by passing the function's signature and documentation to an LLM and asking it to generate the `tool()` call for you. `create_tool_def()` is not perfect, so you must review the generated code before using it. But it is a huge time-saver nonetheless, and removes the boilerplate generation you'd have to do otherwise.
...
Now we need to tell our chat object about our tool. We do this with `$register_tool()`:
