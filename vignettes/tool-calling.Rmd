...
You can also pre-seed the store before chatting, or inspect it afterwards, via `chat$store`:

```{r}
#| eval: false

chat$store$db <- DBI::dbConnect(RSQLite::SQLite(), ":memory:")
```

Note that the store is runtime state: non-serializable handles such as database connections will not survive `saveRDS()`. Use `clone(deep = TRUE)` if you need a chat copy with its own independent store.
