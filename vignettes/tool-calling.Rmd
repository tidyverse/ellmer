...
Note that the store is runtime state: non-serializable handles such as auth tokens or database connections will not survive `saveRDS()`. Use `clone(deep = TRUE)` if you need a chat copy with its own store: this gives the copy a fresh environment with the same top-level bindings, though any nested mutable objects (such as a connection handle) are still shared with the original.
