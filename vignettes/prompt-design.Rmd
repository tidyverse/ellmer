...
```{r}
#| include: false
has_key <- ellmer:::openai_key_exists() && ellmer:::anthropic_key_exists()
has_cassette <- file.exists("_vcr/prompt-design-code-basic.yml")

knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>",
  eval = has_key || has_cassette
)
vcr::setup_knitr()
```
...
# code better match my prose. And make sure echo is suppressed so we can use
# vcr
chat_anthropic <- function(...) {
  ellmer::chat_anthropic(..., params = params(temperature = 0), echo = "none")
}
chat_openai <- function(...) {
  ellmer::chat_openai(..., echo = "none")
}
```
