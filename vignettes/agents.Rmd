...
3.  Iteration that calls repeatedly calls tools and sends results back to the model until there's nothing left to do.

ellmer automatically provides the last property so all you need to do is register the appropriate tools. That means making an agent is surprisingly simple!
...
Now we can make an agent, a chat object that can use these tools:
...
Now we can ask our agent to do stuff for us:
...
I hope this example makes you feel nervous: we've just given an LLM the abililty to make delete files on our computer! This brings us to the most important topic when writing agents: safety and security.
