...
Using these type specifications ensures that the LLM will return JSON. But ellmer goes one step further and converts the results to the closest R analog. Currently, this converts arrays of boolean, integers, numbers, and strings into logical, integer, numeric, and character vectors, and array of objects into data frames. You can opt-out of this and get plain lists by setting `convert = FALSE`.
...
Note that the type functions default to `required = TRUE` which means the LLM will try really hard to extract values for you:
...
In most cases, you'll get a data frame because you are using `parallel_chat_structured()`, with each output row representing one input prompt. But in other cases, you might have a more complex doc where you want a data frame from a single prompt. For example, imagine that you want to extract some data about people from a table:
...
In this case, you might be tempted to use a definition similar to R: an object (i.e., a named list) containing multiple vectors (i.e., an array).
...
This, however, is not quite right because there's no constraint that each array should have the same length, and hence no way for ellmer to know that you really wanted a data frame. Instead, you'll need to turn the data structure "inside out" and create an array of objects:
