...
The following Shiny app uses both `$stream_async()` and `$chat_async()` to stream a story from an OpenAI model.
We ask the user for a prompt to generate a story and then stream the story into the UI as it is generated using `shinychat::markdown_stream()`.
Then we follow up by asking the model for a story title and we use the response to update the card title.

This example highlights the difference between streaming and non-streaming chat.
