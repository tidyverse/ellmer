...
For an asynchronous chat interface with Shiny, we recommend using [the shinychat package](https://posit-dev.github.io/shinychat/) which provides a chat interface for Shiny applications.

You can easily integrate a chat UI similar to the app created by `live_browser()` using shinychat's chat module with the `shinychat::chat_mod_ui()` and `shinychat::chat_mod_server()` functions.
This module connects an `ellmer::Chat` object to `shinychat::chat_ui()` and handles non-blocking asynchronous chat interactions.
...
For fully custom streaming applications, you can use the `shinychat::markdown_stream()` function to stream responses into a Shiny UI.
